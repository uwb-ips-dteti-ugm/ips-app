...
```
**Expected (by design)**: `401` — access and refresh tokens are signed with different secrets (`APP_ACCESS_TOKEN_SECRET` vs `APP_REFRESH_TOKEN_SECRET`), so a refresh token fails signature verification when checked against the access secret.

**ACTUALLY OBSERVED**: `500`, `{"error": "Something went wrong while verifying your session."}`. Same root cause as `AUTH-09` in `01_auth.md` — this `.env` sets both token secrets to the identical placeholder `CHANGE_ME`, so signature verification succeeds. But this path is worse than `AUTH-09`'s: `JwtTokenIssuer.validate_access_token` (`infrastructure/utility/token/jwt.py`) decodes the token successfully (same secret), then builds `UserAccessTokenClaims(user_id=payload["user_id"], role_id=payload["role_id"], name=payload["name"])` — a refresh token's payload only ever has `user_id`+`exp` (`UserRefreshTokenClaims`), so indexing `payload["role_id"]` raises a plain `KeyError`. `validate_access_token` has its own `except Exception as e: raise UnexpectedDomainException(str(e)) from e`, so that `KeyError` becomes an `UnexpectedDomainException` — which is **not** one of the two types (`ExpiredTokenDomainException`/`InvalidTokenDomainException`) `JwtMiddleware.dispatch` specifically catches, so it falls through to the middleware's own generic `except Exception: return 500`. **Confirmed conclusively** by re-testing with distinct secrets (same technique as `AUTH-09`): signature verification then fails immediately, before ever reaching the `KeyError`, producing the originally-expected clean `401`. So there are actually two stacked issues here, not one: (1) the shared-secret `.env` default (same as `AUTH-09`), and (2) even if that's fixed, a token that passes signature checks but is missing an expected claim currently surfaces as an ugly `500` instead of a clean `401` — worth guarding the `payload["role_id"]`/`payload["name"]` access in `validate_access_token` with an explicit conversion to `InvalidTokenDomainException`, so a malformed-but-signature-valid token degrades gracefully regardless of the secret-sharing issue.

**FIXED (both halves)**: (1) `.env`/`.env.example` now use distinct token secrets (see `AUTH-09`). (2) `validate_access_token`/`validate_refresh_token` (`infrastructure/utility/token/jwt.py`) now each wrap their claims construction in a nested `try/except KeyError: raise InvalidTokenDomainException() from None`. Re-verified live: this scenario now returns a clean `401 {"error": "Your session is invalid. Please sign in again."}`.
...
**FIXED**: added `get_by_id(document_cls, id, **kwargs)` to `infrastructure/repository/_shared/object_id.py` — it checks the id is a valid `PydanticObjectId` shape *before* calling `Document.get()`, returning `None` (which the existing `if not doc: raise NotFoundDomainException(...)` at every call site already handles) instead of letting Beanie's internal `parse_object_as` crash on a malformed value. Swapped into every affected `Document.get(to_object_id(id), ...)` call site across `permission`, `role`, `user`, `node`, `node_network`, and `ranging`'s repositories. Re-verified live across all five entities: `GET /permissions/not-an-id`, `/nodes/not-an-id`, `/roles/not-an-id`, `/users/not-an-id`, `/node-networks/not-an-id` now all return `404` instead of `500`.

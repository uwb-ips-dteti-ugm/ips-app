...
**ACTUALLY OBSERVED**: the "stays open regardless" half was confirmed exactly as written. But when run with real fixtures (`node-004` as source, `node-005` as destination, both approved and network-assigned), the measurement was **unexpectedly rejected** — `GET /ranging/latest` returned `null` instead of the recorded measurement — even though every precondition looked satisfied. This is NOT a flaw in this scenario or a flaky test: it's a symptom of a separate, much more serious bug found while investigating it. `node-004`'s network assignment had earlier been changed via `PATCH /nodes/{id}/network` (in `05_node.md`'s `NODE-22`/`NODE-23`/`NODE-25`), which — as documented in full in `05_node.md`'s `NODE-32` — silently corrupts the node's `network` field from a proper Mongo `DBRef` into a fully embedded subdocument, making the node invisible to any lookup that queries `network.$id` (which is exactly how ranging reports resolve source/destination nodes). **See `NODE-32` in `05_node.md` for the full root cause, live reproduction, and blast radius** — this is the single most severe finding across the whole test pass, and this scenario is what surfaced it. A ranging report against a node that has never had its network reassigned via `PATCH` (e.g. one whose network/address were only ever set at creation time) does not hit this problem.

...
namespace infrastructure::ranging::stateless
{
    ...
    models::Error DW3000Impl::initiate(const models::RangingCommand &command, float *distance)
    {
        if (distance == nullptr)
        {
        ...
        fillFrameHeader(tx_poll, sequence_number, command.pan_id, command.listener_address, command.initiator_address, models::RangingFunctionCode::Poll);

        dwt_setrxaftertxdelay(poll_tx_to_resp_rx_delay_uus);
        dwt_setrxtimeout(command.timeout_uus);
        ...
        if (dwt_starttx(DWT_START_TX_IMMEDIATE | DWT_RESPONSE_EXPECTED) != DWT_SUCCESS)
        {
            logger->error(
                initiateTag,
                "Failed to start poll TX (pan_id=0x%04X initiator=0x%04X listener=0x%04X)",
        ...
        if (frame_len < models::RangingFrameLength::Resp ||
            !hasExpectedHeader(rx_resp, command.pan_id, command.initiator_address, command.listener_address, models::RangingFunctionCode::Resp))
        {
            logger->error(
                initiateTag,
    ...
    models::Error DW3000Impl::listen(const models::RangingCommand &command)
    {
        if (command.timeout_uus == 0)
        {
        ...
        if (frame_len < models::RangingFrameLength::Poll ||
            !hasExpectedHeader(rx_poll, command.pan_id, command.listener_address, command.initiator_address, models::RangingFunctionCode::Poll))
        {
            logger->error(
                listenTag,
        ...
        fillFrameHeader(tx_resp, sequence_number, command.pan_id, command.initiator_address, command.listener_address, models::RangingFunctionCode::Resp);
        ...
        if (dwt_starttx(DWT_START_TX_DELAYED) != DWT_SUCCESS)
        {
            logger->error(
                listenTag,
                "Failed to start delayed response TX (pan_id=0x%04X listener=0x%04X initiator=0x%04X)",
                static_cast<unsigned int>(command.pan_id),
                static_cast<unsigned int>(command.listener_address),
                static_cast<unsigned int>(command.initiator_address));
